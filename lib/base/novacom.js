        ...
        getSshPrvKey: function(target, next) {
            ...
            async.waterfall([
                ...
                function(targetDevice, next) {
                    ...
                    const urlStr = 'http://' + targetDevice.host + ':9991/webos_rsa';
                    ...
                    const headOpts = { hostname: url.hostname, port: url.port, path: url.pathname, method: 'HEAD' };
                    const headReq = http.request(headOpts, function(res) {
                        res.resume();
                        ...
                        const writeStream = fs.createWriteStream(keySavePath);
                        http.get(urlStr, function(getRes) {
                            if (getRes.statusCode !== 200) {
                                writeStream.close();
                                fs.unlink(keySavePath, function() {});
                                return setImmediate(next, errHndl.getErrMsg("FAILED_GET_SSHKEY"));
                            }
                            getRes.pipe(writeStream);
                            writeStream.on('finish', function() {
                                writeStream.close(function(error) {
                                    if (error) {
                                        return setImmediate(next, errHndl.getErrMsg("FAILED_GET_SSHKEY"));
                                    }
                            ...
                            writeStream.close();
                            fs.unlink(keySavePath, function() {});
                            setImmediate(next, errHndl.getErrMsg("FAILED_GET_SSHKEY"));
                        });
                    });
                    headReq.on('error', function(err) {
                        setImmediate(next, errHndl.getErrMsg("FAILED_GET_SSHKEY"));
                    });
                    headReq.end();
                },
