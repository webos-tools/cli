...
function explorerController(param, pressed_button, filetype) {
	...
	switch (pressed_button) {
			...
			if (!isRoot) {
				...
				idcap.request("idcap://storage/file/copy", {
					...
					"onFailure": function (err) {
						...
						errorMessage = "Cannot copy selected file : [" + clipboard_path + "] to [" + param + '/' + clipboard_file + "]"
						document.getElementById('fileInfo').innerHTML = errorMessage;
						document.cursor_pos = 0;
						ROOT();
					}
		...
		case BUTTON_3:
			if (!isRoot) {
				idcap.request("idcap://storage/file/move", {
					"parameters": {
						"from": clipboard_path,
						"to": param + '/' + clipboard_file
					},
					...
					"onFailure": function (err) {
						console.log("onFailure : errorMessage = " + err.errorMessage);
						errorMessage = "Cannot move selected file : [" + clipboard_path + "] to [" + param + '/' + clipboard_file + "]";
						document.getElementById('fileInfo').innerHTML = errorMessage;
						document.cursor_pos = 0;
						ROOT();
					}
		...
		case BUTTON_4:
			if (!isRoot) {
				idcap.request("idcap://storage/file/remove", {
					...
					"onFailure": function (err) {
						console.log("onFailure : errorMessage = " + err.errorMessage);
						errorMessage = "Cannot remove selected item : [" + JSON.stringify(param) + "]";
						document.getElementById('fileInfo').innerHTML = errorMessage;
						document.cursor_pos = 0;
						ROOT();
					}
		...
		case BUTTON_6:
			if (!isRoot) {
				...
				idcap.request("idcap://storage/file/unzip", {
					...
					"onFailure": function (err) {
						console.log("onFailure : errorMessage = " + err.errorMessage);
						errorMessage = "Cannot unzip selected file : [" + param + "] to [" + param.slice(0, -4) + "]";
						document.getElementById('fileInfo').innerHTML = errorMessage;
						document.cursor_pos = 0;
						ROOT();
					}
		...
		case BUTTON_7:
			...
			idcap.request("idcap://system/logoImage/set", {
				"parameters": {
					"uri": param
				},
				"onSuccess": function () {
				...
				"onFailure": function (err) {
					console.log("onFailure : errorMessage = " + err.errorMessage);
					errorMessage = "Cannot change logo selected file : [" + param + "]";
					document.getElementById('fileInfo').innerHTML = errorMessage;
					document.cursor_pos = 0;
					ROOT();
				}
